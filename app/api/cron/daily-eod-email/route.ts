  ...
  try {
    ...
    for (const org of organizations) {
      ...
      try {
        ...
        // IDEMPOTENCY CHECK: Atomic insert prevents duplicate emails even with concurrent invocations
        try {
          const isNew = await db.cronExecutions.recordExecution("daily-eod-email", org.id, today, currentHour)
          if (!isNew) {
