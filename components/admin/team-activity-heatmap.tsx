      ...
      <div className={cn("space-y-3", className)}>
        ...
        {memberActivities.map(({ member, activities, totalEod, totalTasks }) => (
          <div key={member.id} className="flex items-center gap-2">
            ...
            {/* Summary */}
            <div className="w-16 shrink-0 text-right text-xs text-slate-500 dark:text-slate-400">
              {totalEod}/{days}
            </div>
          </div>
        ))}
        ...
        {/* Legend */}
        <div className="flex items-center justify-end gap-4 pt-2 text-xs text-slate-500 dark:text-slate-400">
