  ...
  const badge = (
    <div
      className={cn(
        ...
        sizeClasses[size],
        earned
          ? `bg-gradient-to-br from-amber-100 to-amber-200 dark:from-amber-900 dark:to-amber-800 ring-2 ring-amber-400 shadow-lg`
          : "bg-slate-100 dark:bg-slate-800 ring-1 ring-slate-200 dark:ring-slate-700 opacity-50",
        className
      )}
    >
      <Icon
        className={cn(
          iconSizes[size],
          earned ? achievement.badgeColor : "text-slate-400 dark:text-slate-500"
        )}
      />
      {earned && size !== "sm" && (
        <div className="absolute -bottom-1 -right-1 h-4 w-4 bg-green-500 rounded-full flex items-center justify-center ring-2 ring-white dark:ring-slate-900">
